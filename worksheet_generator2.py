...
# Classes
class Worksheet:
# Objects of every other class will ultimately be appended to one of the lists in a worksheet object. This is what gets converted into the HTML worksheet.
...
class Question:
#This is the main way that the worksheets are broken up; a student cannot move to the next question without finishing the current one. Questions contain lists of sections.
...
#A Dragoon object contains the problem name and mode of the Dragoon problem to be inserted.
	def __init__ (self,sect):
		if isinstance(sect,Question): #If the parts of this Dragoon problem have not already been defined, ask for them.
...
def generateHTMLWorksheet(wks):
	...
	qno = 0
	for ques in wks.questions: #This separates the individual questions from each other.
		dragoon_link = ""
		qno = str(int(qno) + 1)
		for lse in wks.loose: #This separates out the loose content.
			if int(qno) == int(lse.before): #This prints the loose content before a given question if that question's number is listed as its "before" value.
				...
				else: #This prints the loose content as text if it is not of the class "LsImage", i.e., if it is of the class "LsText".
					worksheet = worksheet + "\n<p>" + lse.text + "</p>"
		...
		worksheet = worksheet + "\n<h3>" + ques.sub + "</h3>"
		alphindex = 0
		for sect in ques.sections: #This separates the sections in a given question from each other.
			lno = alph[alphindex]
			alphindex = alphindex + 1
			romannum = 0
			for part in sect.content: #This separates the parts in a given section from each other.
				if part.__class__.__name__ == "Dragoon": #If the part is a Dragoon problem, this formats it as such.
				...
				elif part.__class__.__name__ == "Dropdown": #If the part is a dropdown box, this formats it as such.
					rno = rnum[romannum]
					romannum = romannum + 1
					allans = ""
					for item in part.options: #This places all of the listed options between <option> tags.
					...
					if lno == "a" and rno == "i": #If this is the first part of the first section of a question, its row in the results table will display the time the number of seconds the student took for the whole question.
				...
				elif part.__class__.__name__ == "Textbox": #If the part is a textbox, this formats it as such.
					rno = rnum[romannum]
					romannum = romannum + 1
					...
					if lno == "a" and rno == "i": #If this is the first part of the first section of a question, its row in the results table will display the time the number of seconds the student took for the whole question.
						...
						endtbl = endtbl + "</td>\n</tr>"
					disabl = disabl + "\ndocument.getElementById(\""+ qno + lno + rno +"\").disabled=true;"
				elif part.__class__.__name__ == "Table": #If the part is a table, this formats it as such.
					chktbl = ""
					allhead = ""
					allbody = ""
					allline = ""
					for item in part.header: #This prints every item in the header list in bold in a table cell.
						item = "\n<td style=\"border: 2pt black solid\"><b>" + str(item) + "</b></td>"
						allhead = allhead + item
					for body in part.rows: #This separates the rows in a give table object from each other.
						allline = allline + "<tr>"
						for drdn in body: #This separates the cells in a given row from each other.
							if isinstance(drdn, str): #If the cell is a string, it can be printed directly into the cell. If it is not a string, the cell contains at least one dropdown box and must be formatted differently.
								item = "\n<td style=\"border: 2pt black solid\">" + drdn + "</td>"
								allline = allline + item
							else:
								allline = allline + "<td style=\"border: 2pt black solid\">"
								for piece in drdn: #This separates the contents of a cell that contains at least one dropdown box.
									...
									else: #If the item is not a string, it must be printed as a dropdown box.
										allans = ""
										rta = str(piece[1])
										for choice in piece[0]: #This places all of the listed options between <option> tags.
				...
				elif part.__class__.__name__ == "Text": #If the item is a piece of text, this formats it as such.
					worksheet = worksheet + part.text
				elif part.__class__.__name__ == "Image": #If the item is an image, this formats it as such.
					worksheet = worksheet + "\n</p><img src =\"" + part.image + ".JPG\"/><p>"
				elif part.__class__.__name__ == "Checkbox": #If the item is a checkbox, this formats it as such.
					rno = rnum[romannum]
					romannum = romannum + 1
					allans = ""
					wrngcb = []
					for item in part.options_c: #This separates the options from each other.
						if not item in part.correct_c: #This creates a list of answers that are not supposed to be checked.
							wrngcb.append(item)
						item = "\n<input type=\"checkbox\" name=\"" + qno + lno + rno + "\" id=\"" + str(item) + "\">" + str(item) + "<br>"
						allans = allans + item
						chkbox = "1"
					for item in part.correct_c: #This separates the correct answers from each other.
					...
					endfn = endfn + "\ndocument.getElementById(\"" + qno + lno + rno + "Tries\").innerHTML=set" + qno + "." + lno + rno + ";"
			worksheet = worksheet + "</p>"
		if int(qno) < len(list(wks.questions)): #If the question is the last question in the worksheet, it needs to be formatted differently.
