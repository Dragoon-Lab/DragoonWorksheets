...
Start by creating an instance of the class `Worksheet`. Each instance of a class is an object. To create an instance of a class, type: [name] = [class]([arguments]). For example, typing, `wkst = Worksheet("Intro_Worksheet")` will create an object of class `Worksheet` with the name `wkst` and with `"Intro_Worksheet"` as its only argument. An argument is an input value for a function. The reason for its use here is that each class has an initialization function that is called whenever a new instance of that class is created. To add contents to the worksheet, create instances of the classes listed below. Once the worksheet is complete, convert it into a JSON file using the `generateJSON` function to save it. JSON is a way of storing objects so that both humans and computers can easily read them. This step is not necessary for creating a worksheet, but it allows for editing of the content of the worksheet without creating a worksheet object from scratch. From this point, you can generate a [worksheet](worksheet.md) using the `generateHTMLWorksheet` function. If the worksheet object is still open, the function can take its name as an argument. If the worksheet object has been converted to a JSON file, this function can take the function `loadWorksheet` as an argument, where the argument for `loadWorksheet` is the name of the JSON file (including ".json").
